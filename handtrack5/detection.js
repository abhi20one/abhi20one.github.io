...
let fingersOpenTimer = 0;
let allFingersOpen = false;
let lastvalue = true;
function ifFiveFingersOpen(handLandmarks) {
    const thumbOpen = angleBetween(handLandmarks[1], handLandmarks[2], handLandmarks[3]) > 170;  
    const indexOpen = angleBetween(handLandmarks[5], handLandmarks[6], handLandmarks[7]) > 170;  
    const middleOpen = angleBetween(handLandmarks[9], handLandmarks[10], handLandmarks[11]) > 170;  
    const ringOpen = angleBetween(handLandmarks[13], handLandmarks[14], handLandmarks[15]) > 170;  
    const pinkyOpen = angleBetween(handLandmarks[17], handLandmarks[18], handLandmarks[19]) > 170;  

    allFingersOpen = thumbOpen && indexOpen && middleOpen && ringOpen && pinkyOpen;


    console.log(allFingersOpen);
    ...
    if (allFingersOpen) {
        ...
        fingersOpenTimer += 1 / 60;
        if (fingersOpenTimer >= 0.1) {
            if (lastvalue) {
                toggleUI();
                lastvalue=false;
            }else{
...
function gotHands(results) {
    ...
    if (detections.multiHandLandmarks && detections.multiHandLandmarks.length > 0) {
        ...
        // Check if all five fingers are open and handle the toggle UI logic
        ifFiveFingersOpen(handLandmarks);
